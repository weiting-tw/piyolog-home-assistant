...
直近に登録されたイベントを削除します。直近の判定にはイベント日時ではなく、ぴよログへの登録時刻が使われます。
誤削除を防ぐため、対象イベントが `max_age_minutes` より古い場合は削除されません。
...
data:
  event_type:
    - pee
    - poo
  max_age_minutes: 10 # 省略時は10分、最大180分
  sync_before_check: true # 省略時はtrue
```
...
| パラメータ          | 値（value）                          | 説明                                                          |
| ------------------- | ------------------------------------ | ------------------------------------------------------------- |
| `event_type`        | 登録可能なイベント種類（複数指定可） | 削除対象のイベント種別。省略時は全種別                        |
| `max_age_minutes`   | `1` 〜 `180`                         | 登録時刻が指定分数より古いイベントは削除しない。省略時は `10` |
| `sync_before_check` | `true`, `false`                      | 削除対象を探す前にぴよログAPIと同期する。省略時は `true`      |
| `baby_id`           | string                               | 対象の赤ちゃんID（任意）                                      |
| `baby_index`        | integer                              | 赤ちゃんのインデックス（0始まり・任意）                       |
